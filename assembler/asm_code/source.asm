...
; numbers
.equ  c_tile_bg     = 0x01    ; background tile number (creeper!!!)
...
; addresses
.equ  a_song = 406            ; address to song start in data space
.equ  a_q = 397               ; q key
.equ  a_w = 398               ; w key
.equ  a_e = 399               ; e key
.equ  a_r = 400               ; r key
...
INIT:
  ...
  lds   r_chord_note,a_song   ; store note for first row
  lds   r_chord_keys,a_song   ; store keys required for first row
...
INPUT:
  ...
  or    r_qwert,r_qwert,r_tmp0; load q state in qwert
...
INSIDE_INTERVAL:
  ...
  brne  CHORD_FAIL            ; chord is failed if keys pressed are not correct
...
DRAW:
  ...
  ldi   r_y,c_disp_height       ; where to draw current chord, y
...
NO_NOTE:
  ...
  inc   r_tmp1
  inc   r_tmp2
  lsr   r_tmp5, 1                ; go to next note on chord
  subi  r_x,r_x,c_note_dist     ; move to position of next note
  ...
  subi  r_y,r_y,c_chord_dist    ; move to position of next chord
